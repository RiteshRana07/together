...
export default function VideoPlayer({videoUrl,channel,broadcast,canControl,onPlaybackError,autoplayOnSourceChange=false,initialSync}){
 ...
 return <div className="relative rounded-xl overflow-hidden bg-black shadow-2xl shadow-black/50">
   <video ref={videoRef} src={videoUrl} controls={canControl} playsInline referrerPolicy="no-referrer" preload="auto" className="w-full aspect-video" onWaiting={()=>{buffering.current=true;setBufferingUi(true)}} onCanPlay={()=>{buffering.current=false;setBufferingUi(false)}} onPlaying={()=>{buffering.current=false;setBufferingUi(false);if(!sourceChanging.current&&Date.now()>=remoteGuard.current)emit("play")}} onPause={()=>{if(!sourceChanging.current&&!buffering.current&&Date.now()>=remoteGuard.current)emit("pause")}} onSeeked={()=>{if(!sourceChanging.current&&Date.now()>=remoteGuard.current&&canControl)emit("seek")}} onError={(e)=>{buffering.current=false;setBufferingUi(false);const err=e.currentTarget?.error;setStreamError(err?.message||`The video stream could not be loaded (media error ${err?.code||"unknown"}).`);onPlaybackError?.()}}/>
