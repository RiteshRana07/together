function normalizeBaseUrl(value) {
  ...
  try {
    const u = new URL(raw);
    // In the common local setup the API/backend may run on 8080 while
    // Next.js serves the clickable web UI on 3000. Email links must open
    // the web UI, not a backend port that may not be exposed.
...
function getPublicAppUrl(request) {
  ...
  // Only use the request origin as a last resort. This keeps production
  // links on the public web origin when APP_URL is configured, while still
  // making local development work.
...
async function sendVerificationEmail({ email, username, token, appUrl }) {
  const baseUrl = normalizeBaseUrl(appUrl) || APP_URL;
