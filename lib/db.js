...
async function deleteUserAccount(userId) {
  ...
  try {
    ...
    // Remove user-owned realtime/personal rows that can otherwise block the
    // final users DELETE because their foreign keys intentionally remain
    // strict. Messages/signals are ephemeral communication data, so deleting
    // them is safer than weakening the foreign keys or leaving orphaned data.
    await client.query(`DELETE FROM room_signals WHERE sender_id=$1 OR target_user_id=$1`, [userId]);
    await client.query(`DELETE FROM room_messages WHERE user_id=$1`, [userId]);
