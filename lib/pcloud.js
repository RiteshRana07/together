...
/*
 * PRIVATE DIRECT VIDEO PLAYBACK
 *
 * IMPORTANT:
 * Use pCloud's authenticated getvideolinks endpoint for WatchTogether.
 * It returns an actual media URL for the file/variant. Do NOT convert this
 * to a public-link URL: public video/download endpoints can return a pCloud
 * web/download response that the HTML5 <video> element cannot stream.
 *
 * Every call is intentionally fresh. pCloud media URLs are temporary and
 * must never be cached in the Railway process.
 */
async function getFreshPlayableVideoLink(fileId) {
  ...
  try {
    ...
    if (variants.length) {
      ...
      const source = compatible.length ? compatible : variants;
      const underLimit = source.filter((v) => {
        const height = Number(v.height) || 0;
        return !height || height <= PLAYBACK_MAX_HEIGHT;
      });

      const pool = underLimit.length ? underLimit : source;
      ...
      if (selected) {
        console.log("[pCloud] fresh video variant selected:", {
          fileId: id,
        ...
        return buildContentUrl(selected);
      }
    }
  } catch (error) {
    console.warn(
      "[pCloud] getvideolinks unavailable; falling back to getfilelink:",
      error?.message || error
    );
  }

  // Original-file fallback. This is still a fresh URL and is preferable to
  // returning a stale URL or a public pCloud web/download endpoint.
  return getFileLink(id);
}

// Compatibility name used by older API routes.
async function getPlayableVideoLink(fileId) {
  return getFreshPlayableVideoLink(fileId);
}

async function getPublicVideoLink(fileId) {
  // IMPORTANT for Railway/production:
  // pCloud content URLs are temporary. Never cache the generated content URL
  // in process memory. Localhost is normally a single long-lived process,
  // while Railway may restart or run multiple instances, which can make a
  // cached content URL stale and produce HTTP 410 Gone in the browser.
  ...
  try {
    ...
    if (variants.length) {
      ...
      const underLimit = (compatible.length ? compatible : variants).filter((v) => {
        const height = Number(v.height) || 0;
        return !height || height <= PLAYBACK_MAX_HEIGHT;
      });
      const pool = underLimit.length
        ? underLimit
        : (compatible.length ? compatible : variants);

      // Prefer the highest compatible quality at or below the configured
      // ceiling. This keeps the browser on a normal H.264/AAC MP4 path.
      pool.sort((a, b) => {
        const aScore = (Number(a.width) || 0) * (Number(a.height) || 0);
        const bScore = (Number(b.width) || 0) * (Number(b.height) || 0);
        return bScore - aScore;
    ...
    console.warn(
      "[pCloud] public video variants unavailable; using public download link:",
      error?.message || error
    );
  }
