...
/*
 * Prefer pCloud's video variants for playback.  This is important for
 * videos uploaded from phones because a phone may upload HEVC/H.265 or
 * another codec that a desktop/mobile browser cannot decode directly.
 * pCloud's getvideolinks can provide browser-friendly H.264 variants.
 */
async function getVideoLink(fileId) {
  try {
    ...
    if (!variants.length) return getFileLink(fileId);
    ...
    // Some files may not have generated video variants yet. Fall back to
    // the original file link instead of making an otherwise valid library
    // item unusable.
    console.warn("[pCloud] getvideolinks failed; falling back to getfilelink:", error?.message || error);
