...
// Direct public playback avoids proxying every video byte through Railway.
// Prefer a 720p-or-lower transcoded pCloud variant for smoother playback.
const PLAYBACK_MAX_HEIGHT = Math.max(360, Number(process.env.PCLOUD_PLAYBACK_MAX_HEIGHT || 720));
const publicPlaybackCache = new Map();
...
// Production-safe fresh original-file link. Never cache this URL because
// pCloud content URLs are temporary and can return 410 after expiry.
...
async function getPublicVideoLink(fileId) {
  ...
  const cached = publicPlaybackCache.get(key);
  ...
  try {
    ...
    if (variants.length) {
      ...
      const candidates = (playable.length ? playable : variants).filter((v) => {
        const height = Number(v.height) || 0;
        return !height || height <= PLAYBACK_MAX_HEIGHT;
      });
      const pool = candidates.length ? candidates : (playable.length ? playable : variants);
      pool.sort((a, b) => {
        const aScore = (Number(a.width) || 0) * (Number(a.height) || 0);
        const bScore = (Number(b.width) || 0) * (Number(b.height) || 0);
        return bScore - aScore;
      });
      const url = buildContentUrl(pool[0]);
      // pCloud playback URLs are refreshed periodically. This keeps Railway
      // from proxying every byte while avoiding a permanently stale URL.
      publicPlaybackCache.set(key, { url, expiresAt: Date.now() + 5 * 60 * 1000 });
      return url;
    }
  } catch (error) {
  ...
  const url = buildContentUrl(download);
  publicPlaybackCache.set(key, { url, expiresAt: Date.now() + 5 * 60 * 1000 });
  return url;
}

async function getPlayableVideoLink(fileId) {
  const id = String(fileId);
  const cached = publicPlaybackCache.get(`private:${id}`);
  if (cached && cached.expiresAt > Date.now()) return cached.url;

  try {
    const data = await api("getvideolinks", {
      fileid: id,
      skipfilename: 1,
    });
    const variants = Array.isArray(data.variants) ? data.variants : [];
    if (variants.length) {
      const compatible = variants.filter((v) => {
        const vc = String(v.videocodec || "").toLowerCase();
        const ac = String(v.audiocodec || "").toLowerCase();
        return vc === "h264" && (ac === "aac" || ac === "mp3");
      });
      const under720 = (compatible.length ? compatible : variants).filter((v) => {
        const h = Number(v.height) || 0;
        return !h || h <= PLAYBACK_MAX_HEIGHT;
      });
      const pool = under720.length ? under720 : (compatible.length ? compatible : variants);
      pool.sort((a, b) => {
        const ap = (Number(a.width) || 0) * (Number(a.height) || 0);
        const bp = (Number(b.width) || 0) * (Number(b.height) || 0);
        return bp - ap;
      });
      const url = buildContentUrl(pool[0]);
      publicPlaybackCache.set(`private:${id}`, { url, expiresAt: Date.now() + 4 * 60 * 1000 });
      return url;
    }
  } catch (error) {
    console.warn("[pCloud] getvideolinks unavailable; falling back to getfilelink:", error?.message || error);
  }
...
async function getPlayableVideoSource(fileId) {
  const id = String(fileId);
  const data = await api("getvideolinks", { fileid: id, skipfilename: 1, contenttype: "video/mp4" });
  const variants = Array.isArray(data.variants) ? data.variants : [];
  if (!variants.length) throw new Error("pCloud returned no video variants.");
  ...
  if (!compatible.length) {
    const summary = variants.map(v => `${v.width || "?"}x${v.height || "?"} ${v.videocodec || "?"}/${v.audiocodec || "?"}`).join(", ");
    const error = new Error(`No browser-compatible H.264 video variant. Available: ${summary}`);
    error.code = "UNSUPPORTED_VIDEO_CODEC";
    throw error;
  }
  const under = compatible.filter(v => !v.height || Number(v.height) <= PLAYBACK_MAX_HEIGHT);
  const pool = under.length ? under : compatible;
  pool.sort((a,b) => ((Number(b.width)||0)*(Number(b.height)||0))-((Number(a.width)||0)*(Number(a.height)||0)));
  const chosen = pool[0];
  return {
    url: buildContentUrl(chosen),
    codec: `${String(chosen.videocodec).toLowerCase()}/${String(chosen.audiocodec).toLowerCase()}`,
    width: Number(chosen.width) || 0,
    height: Number(chosen.height) || 0,
    expires: chosen.expires || null,
  };
}

// Fresh playable link for server-side streaming. Unlike getPlayableVideoLink(),
// this intentionally does not reuse the short-lived content URL cache. The
// stream proxy may receive byte-range requests long after the previous URL was
// generated, so a stale pCloud URL can otherwise return 410 Gone.
async function getFreshPlayableVideoLink(fileId) {
  const id = String(fileId);
  try {
    const data = await api("getvideolinks", { fileid: id, skipfilename: 1 });
    const variants = Array.isArray(data.variants) ? data.variants : [];
    if (variants.length) {
      const compatible = variants.filter((v) => {
        const vc = String(v.videocodec || "").toLowerCase();
      ...
      return buildContentUrl(pool[0]);
    }
  } catch (error) {
    console.warn("[pCloud] fresh getvideolinks unavailable; falling back to fresh getfilelink:", error?.message || error);
  }
...
module.exports = {
  ...
  getFileLink,
  getFreshFileLink,
  getPlayableVideoLink,
  getFreshPlayableVideoLink,
