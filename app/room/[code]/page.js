...
export default function RoomPage({params}){
 ...
 async function resolveRoomPlayback(ref, fallbackUrl, cancelledRef){
   ...
   const response=await fetch(`/api/storage/playback-url?room=${encodeURIComponent(code)}&v=${encodeURIComponent(ref)}&_=${Date.now()}`,{cache:'no-store',credentials:'include'});
   const data=await response.json().catch(()=>({}));
   if(!response.ok||!data.url) throw new Error(data.error||`Could not create playback URL (HTTP ${response.status})`);
   return data.url;
 }
