...
export default function RoomPage({params}){
 ...
 async function resolveRoomPlayback(ref, fallbackUrl, cancelledRef){
   ...
   // Never block the room UI on pCloud public-link creation. The stream route
   // authenticates the room member and immediately redirects the browser to a
   // fresh pCloud content URL, so the video can start without a long
   // “Preparing the video stream…” state.
   ...
   <div className="grid xl:grid-cols-[minmax(0,1fr)_390px] gap-5">
    <section className="min-w-0">
      <div className="rounded-3xl overflow-hidden border border-white/[.1] bg-black shadow-2xl relative">
       {!playbackReady?<div className="aspect-video flex items-center justify-center bg-black text-white/45 text-sm">Preparing the video stream…</div>:currentVideoSource==='youtube'?<YouTubePlayer videoId={currentVideoUrl} channel={channel} broadcast={broadcast} canControl={isHost} initialSync={{time:playback.time||0,playing:!!playback.playing,updatedAt:playback.updatedAt||0}} autoplayOnSourceChange={autoplay}/>:<VideoPlayer videoUrl={currentVideoUrl} channel={channel} broadcast={broadcast} canControl={isHost} autoplayOnSourceChange={autoplay} initialSync={{time:playback.time||0,playing:!!playback.playing,updatedAt:playback.updatedAt||0}} onPlaybackError={hydrate}/>} 
