...
export default function RoomPage({params}){
 ...
 useEffect(()=>{
  ...
  const load=async()=>{
    try{
      ...
      const playable=isPCloudRef(ref)?'':(rd.room.playable_current_video_url||rd.room.playable_video_url||ref);
 ...
 async function resolveRoomPlayback(ref, fallbackUrl, cancelledRef){
   ...
   // Keep pCloud completely behind the same-origin stream endpoint. The
   // endpoint authenticates the room member, creates a fresh pCloud link for
   // every media request, sends pCloud's required referrer server-side and
   // forwards Range/206 responses to the browser.
   return `/api/storage/stream?room=${encodeURIComponent(code)}&v=${encodeURIComponent(ref)}&_=${Date.now()}`;
 }
...
p=getPusherClient();onConnected=()=>setSocketId(p.connection.socket_id);p.connection.bind('connected',onConnected);if(p.connection.state==='connected')onConnected();ch=p.subscribe(name);ch.bind('pusher:subscription_succeeded',m=>{const list=[];m.each(x=>list.push({id:x.id,username:x.info.username,isHost:!!x.info.isHost}));if(!cancelled&&list.length)setParticipants(list)});ch.bind('pusher:member_added',m=>setParticipants(x=>x.some(q=>String(q.id)===String(m.id))?x:[...x,{id:m.id,username:m.info?.username||'Guest',isHost:!!m.info?.isHost}]));ch.bind('pusher:member_removed',m=>setParticipants(x=>x.filter(q=>String(q.id)!==String(m.id))));ch.bind('pusher:subscription_error',()=>setJoinError('Realtime connection unavailable. The room is using database sync.'));ch.bind('room:video-changed',async d=>{const ref=d.videoRef||d.videoUrl||'';if(cancelled)return;try{const url=await resolveRoomPlayback(ref,d.videoUrl||ref,false);if(cancelled)return;setAutoplay(!!d.autoplay);setRoom(r=>r?({...r,current_video_url:ref,current_video_title:d.videoTitle||r.current_video_title,current_video_source:d.videoSource||r.current_video_source,playback_time:0,playback_playing:!!d.autoplay,playback_updated_at:Date.now()}):r);setPlayback({url,title:d.videoTitle||'',source:d.videoSource||'',ref,version:Date.now(),time:0,playing:!!d.autoplay,updatedAt:Date.now()});setPlaybackReady(true)}catch(e){if(!cancelled)setJoinError(e.message||'Could not load the selected video.')}});ch.bind('room:capacity-changed',d=>setRoom(r=>({...r,max_participants:d.maxParticipants})));setChannel(ch)}catch(error){setJoinError('Realtime service unavailable. The room is using database sync.')}return()=>{cancelled=true;try{if(p){p.unsubscribe(name);if(onConnected)p.connection.unbind('connected',onConnected)}}catch{}setChannel(null)}},[user?.id,room?.id,code,joinState]);
 useEffect(()=>{if(joinState!=='joined'||!room)return;let cancelled=false;const run=async()=>{const ref=room.current_video_url||room.video_url||'';const source=room.current_video_source||room.video_source||'';try{const url=await resolveRoomPlayback(ref,room.playable_current_video_url||room.playable_video_url||ref,false);if(cancelled)return;setPlayback(p=>({...p,url,ref,source,version:p.version+1}));setPlaybackReady(true)}catch(e){if(cancelled)return;setPlaybackReady(true);setPlayback(p=>({...p,url:'',ref,source,version:p.version+1}));setJoinError(e.message||'Could not load the video stream.')}};run();return()=>{cancelled=true}},[joinState,room?.id,room?.current_video_url,room?.video_url,room?.current_video_source,room?.video_source,room?.playable_current_video_url,room?.playable_video_url,code]);
 ...
 async function playOriginal(){const r=await fetch(`/api/rooms/${code}/queue/original`,{method:'POST'});const d=await r.json();if(!r.ok)return alert(d.error||'Could not restore original');const ref=d.videoRef||d.room.original_video_url||d.room.video_url;const url=await resolveRoomPlayback(ref,d.playableVideoUrl,false);setAutoplay(true);setPlaybackReady(true);setRoom(x=>x?({...x,...d.room,playback_time:0,playback_playing:true,playback_updated_at:Date.now()}):x);setPlayback({url,title:d.room.original_video_title||d.room.video_title,source:d.room.original_video_source||d.room.video_source,ref,version:Date.now(),time:0,playing:true,updatedAt:Date.now()});setPlaybackReady(true)}
   ...
   <div className="grid xl:grid-cols-[minmax(0,1fr)_390px] gap-5">
    <section className="min-w-0">
      <div className="rounded-3xl overflow-hidden border border-white/[.1] bg-black shadow-2xl relative">
       {!playbackReady?<div className="aspect-video flex items-center justify-center bg-black text-white/45 text-sm">Preparing the video stream…</div>:currentVideoSource==='youtube'?<YouTubePlayer videoId={currentVideoUrl} channel={channel} broadcast={broadcast} canControl={isHost} initialSync={{time:playback.time||0,playing:!!playback.playing,updatedAt:playback.updatedAt||0}} autoplayOnSourceChange={autoplay}/>:<VideoPlayer videoUrl={currentVideoUrl} channel={channel} broadcast={broadcast} canControl={isHost} autoplayOnSourceChange={autoplay} initialSync={{time:playback.time||0,playing:!!playback.playing,updatedAt:playback.updatedAt||0}} onPlaybackError={async()=>{try{const ref=playbackRef.current||room?.current_video_url||room?.video_url||'';if(!ref)return;const url=await resolveRoomPlayback(ref,'',false);setPlayback(p=>({...p,url,version:p.version+1}));setPlaybackReady(true)}catch(e){setJoinError(e.message||'Could not refresh the video stream.')}}}/>} 
